...
namespace Igor.TCP {
	...
	public partial class MultipleClientsPerServer {
		...
		public async Task ConnectingMultipleClients() {
			...
			Assert.IsTrue(server.ConnectedClients.Single(w => w.ID == client1.Info.ID).Name == "Client 1");
			...
			Assert.IsTrue(server.ConnectedClients.Single(w => w.ID == client2.Info.ID).Name == "Client 2");
